...
If the ticket instead asks for new capability that never existed, use `feature-implementation.md` instead.
...
**Working in a git worktree?** (See `WORKTREE-WORKFLOW.md`.) This skill yields the `bug/` branch prefix. When the orchestrator delegates the confirmed plan to `catalyst-impl`, the implementation agent creates the worktree at `../<repo>-<ticketid>` on branch `bug/{ticketid}_{summary-slug}` and does the implementation/tests/PR inside it. If the ticket has no ID (a bare description), it uses `bug/{summary-slug}` and `../<repo>-{summary-slug}` instead. Tracker-agnostic: nothing here depends on which tracker the ticket came from.
