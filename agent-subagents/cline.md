...
To apply Catalyst's roles here, define six team members in your Cline SDK session config:
...
- **`catalyst-impl`** — a team member with full read/write/execute tools and your highest-capability model, given the confirmed plan, the ticket, and the relevant skill file. Creates its own worktree, executes the plan, reports results. Same system prompt as `agent-subagents/claude-code/catalyst-impl.md`.
- **`catalyst-code-reviewer`** — a read-only team member with a high-capability model, given the ticket and the implemented changes after IMPLEMENT, reviewing for bugs, style (repo skills first, else industry best practice), and accuracy against the ticket. Returns a compact structured report. Same system prompt as the Claude Code code-reviewer file.
...
- For IMPLEMENT, after the plan is confirmed, spawn one CLI instance with the implementation agent's instructions from `agent-subagents/claude-code/catalyst-impl.md`, giving it the confirmed plan, the ticket, and the skill file. It creates its own worktree, executes the plan, and reports back.
