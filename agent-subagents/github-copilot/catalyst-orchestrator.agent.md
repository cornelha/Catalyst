...
1. Read the ticket — if given as a bare ID (e.g. #4521), resolve it to full ticket content first via the tracker's configured MCP tool, passing the numeric ID only (strip any `#` or URL wrapper). Then read `ORCHESTRATION-PROMPT.md` and check `catalyst-skills/` for a matching skill file; if one exists, use its Analysis Tasks / Verification Questions / Implementation Checklist as your delegation basis.
