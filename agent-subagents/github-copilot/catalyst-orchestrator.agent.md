...
2. FAN OUT: decompose the ticket into independent analysis tasks. Delegate each to the `catalyst-fan-out-analyst` custom agent as a subagent (or use Copilot CLI Fleet mode for true parallel execution) — each gets only its single task description.
