...
PHASE 1 — FAN OUT
State the independent analysis tasks you'll run. List them before running them. Then actually run them (search the codebase, read files, check git history, etc.) — genuinely in parallel where the tooling allows, not narrated-as-parallel-but-actually-sequential. If this ticket type is unfamiliar, cap the first run (e.g. 5-8 tasks) so an unknown ticket can't silently burn unbounded tokens.
