...
PHASE 1 — FAN OUT
State the independent analysis tasks you'll run. List them before running them. Then actually run them (grep/search the codebase, read files, check git history, etc.) — batch independent read-only bash commands into as few calls as possible rather than interleaving them with analysis. If this ticket type is unfamiliar, cap the first run (e.g. 5-8 tasks) so an unknown ticket can't silently burn unbounded tokens.
...
Stop after the plan and ask for confirmation before applying any patch.
