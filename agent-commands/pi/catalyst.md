...
PHASE 1 — FAN OUT
State the independent analysis tasks you'll run. List them before running them. Then actually run them (grep/search the codebase, read files, check git history, etc.) — batch independent read-only bash commands into as few calls as possible rather than interleaving them with analysis.
